...
async def parse_details(data, matchId, platform):
    ...
    if data["info"]["queueId"] == 0:
        return {"found": False, "platform": platform, "data": {"matchId": matchId}}
    ...
    if "gameStartTimestamp" in data["info"] and "gameEndTimestamp" in data["info"]:
        game_duration = (
...
async def parse_summoners(data, platform):
    ...
    return {
        ...
        "platform": platform,
        "summoners": [
            {"name": player["summonerName"], "puuid": player["puuid"]}
