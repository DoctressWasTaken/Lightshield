...
class Mapping:
            ...
            for cat in ep:
                for endpoint in ep[cat]:
                    ...
                    for plc in re.findall('\{([a-zA-Z]*)}', endpoint):
                        regex = regex.replace('{%s}' % plc, placeholders[plc].replace('/', '\\/'))
                    self.endpoints[regex] = endpoint
        # pp.pprint(self.endpoints)

    async def init(self, host="localhost", port=6379):
        self.session = aiohttp.ClientSession(headers={"X-Riot-Token": os.environ.get('API_KEY')})
    ...
    async def pre_flight(self, server, endpoint, send_timestamp, request_id):
        ...
        time_to_wait = await self.redis.evalsha(self.permit, 2,
                                                server, endpoint,  # Keys
                                                send_timestamp, request_id)  # Args
