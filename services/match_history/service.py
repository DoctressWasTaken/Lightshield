...
class Platform:
    ...
    def __init__(self, region, platforms, handler):
        ...
        self.endpoint_url = f"https://{self.name}.api.riotgames.com" \
    ...
    async def runner(self):
        ...
        try:
            ...
            await self.flush_tasks(list(set(self.result_matchids)), self.result_summoners)
            return
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                while starts:
                    ...
                    starts = [start for start in (await asyncio.gather(*[
                        asyncio.create_task(self.fetch(target['puuid'], start, session))
                        for start in starts
                    ])) if start]
        except Exception as err:
        ...
        try:
            async with self.handler.postgres.acquire() as connection:
                if matches or summoner:
                    self.logging.info("Flushing %s match ids and %s summoner updates.", len(matches), len(summoner))
                if matches:
                    splits = []
