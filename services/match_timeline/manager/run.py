...
class Manager:
    ...
    async def init(self):
        ...
        await self.redis.delete("%s_match_timeline_in_progress" % settings.SERVER)
        await self.redis.delete("%s_match_timeline_tasks" % settings.SERVER)
    ...
    async def run(self):
        ...
        try:
            while not self.stopped:
                ...
                self.logging.info(
                    "Filled tasks to %s.",
                    await self.redis.scard("%s_match_timeline_tasks" % settings.SERVER),
                )
