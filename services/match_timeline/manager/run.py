...
class Manager:
    ...
    async def init(self):
        async with self.redis.get_connection() as buffer:
            await buffer.delete("%s_match_timeline_in_progress" % self.server)
            await buffer.delete("%s_match_timeline_tasks" % self.server)
    ...
    async def get_tasks(self):
        ...
        async with self.db.get_connection() as db:
            tasks = await db.fetch(
                ...
                ORDER BY CASE WHEN details IS NOT NULL THEN 1 ELSE 0 END DESC, timestamp DESC
                LIMIT $1;
                """
            ...
            while not self.stopped:
                ...
                async with self.redis.get_connection() as buffer:
                    await buffer.zremrangebyscore(
                        "%s_match_timeline_in_progress" % self.server, max=limit
                    )
                    ...
                    for entry in result:
                        ...
                        # Insert task hook
                        await buffer.sadd(
                            "%s_match_timeline_tasks" % self.server, entry["match_id"]
                        )

                    self.logging.info(
                        "Filled tasks to %s.",
                        await buffer.scard("%s_match_timeline_tasks" % self.server),
                    )
