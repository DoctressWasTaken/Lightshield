...
class Manager:
    ...
    async def init(self):
        async with self.redis.get_connection() as buffer:
            await buffer.delete("%s_match_timeline_in_progress" % settings.SERVER)
            await buffer.delete("%s_match_timeline_tasks" % settings.SERVER)
            ...
            while not self.stopped:
                ...
                async with self.redis.get_connection() as buffer:
                    await buffer.zremrangebyscore(
                        "%s_match_timeline_in_progress" % settings.SERVER, max=limit
                    )
                    # Check remaining buffer size
                    if (
                        size := await buffer.scard(
                            "%s_match_timeline_tasks" % settings.SERVER
                        )
                    ...
                    for entry in result:
                        ...
                        if await buffer.zscore(
                            "%s_match_timeline_in_progress" % settings.SERVER,
                            entry["match_id"],
                        ):
                            continue
                        # Insert task hook
                        await buffer.sadd(
                            "%s_match_timeline_tasks" % settings.SERVER, entry["match_id"]
                        )

                    self.logging.info(
                        "Filled tasks to %s.",
                        await buffer.scard("%s_match_timeline_tasks" % settings.SERVER),
                    )
