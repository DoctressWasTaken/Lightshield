...
class RabbitManager:
    ...
    def __init__(self, exchange=""):
        ...
        self.blocked = (
            False  # Set to true if the queue rejects messages due to being full.
        )
        self.stopped = (
            False  # Set to true once the service receives the shutdown signal
        )
        ...
        self.outstanding_messages = (
            []
        )  # Contains outstanding ready messages rejected by the queue
        ...
        try:
            ...
            self.logging.info(
                "Restarted service with %s outstanding tasks."
                % len(self.outstanding_messages)
            )
        except:
            pass
    ...
    async def connect(self):
        ...
        self.exchange = await self.channel.declare_exchange(
            name=self.exchange_name, durable=True, type=ExchangeType.TOPIC, robust=True
        )
        ...
        try:
            try:
                ...
                if not self.check_queue_task:
                    self.check_queue_task = asyncio.create_task(self.check_queue())
